...
//衛星用に関数化
void drawSatellite(double distance, double size, double orbitPeriod, double rotationPeriod)//衛星を描画する関数
{
    glPushMatrix();//現時点での行列を保存
	glRotated((double)year / orbitPeriod, 0.0, 1.0, 0.0);//衛星の公転
	glTranslated(distance, 0.0, 0.0);//距離を設定
    glRotated((double)day / rotationPeriod, 0.0, 1.0, 0.0);//衛星の自転
	glutWireSphere(size, 10, 8);//衛星を描画
	glPopMatrix();//行列を復元
}
//いちいち書くのもめんどくさいから関数化
void drawPlanet(double distance, double size, double yearRotation, double dayRotation, double satelliteData[][4], int numSatellites)//引数の意味はそれぞれ、距離、サイズ、公転周期、自転周期、衛星のデータ、衛星の数
{
	glPushMatrix();////現時点での行列を保存
...
void myDisplay(void)
{
	...
	double earthSatellites[][4] = { {0.7, 0.04, 27.32, 27.32} }; // 月の公転周期と自転周期
    double marsSatellites[][4] = { {0.5, 0.05, 0.319, 0.319}, {0.5, 0.05, 1.2624, 1.2624} }; // フォボスとダイモス
    double jupiterSatellites[][4] = { {1.2, 0.1,7.155,7.155}, {1.5, 0.1, 16.689, 16.689} }; // ガニメデとカリスト
    double saturnSatellites[][4] = { {0.8, 0.05, 4.12, 4.12}, {0.8, 0.05, 15.945, 15.945} }; //レアとタイタン

	//水星
	drawPlanet(4.0, 0.2, 0.24, 58.65, NULL, 0);//引数の意味はそれぞれ、距離、サイズ、公転周期、自転周期
	...
	double X = 24.0 * cos(year * PI / 247.74);//楕円の媒介変数表示　x座標
	double Z = 24.0 * sin(year * PI / 247.74) * 0.8; //楕円の媒介変数表示　z座標
	glTranslated(X, 0.0, Z);//距離引数は楕円軌道のx座標とz座標　Yが動かないのはxz平面上に描画するため
	glRotated((double)day / 6.39, 0.0, 1.0, 0.0);//自転
	glutWireSphere(0.1, 10, 8);//冥王星を描画
	glPopMatrix();//行列を復元
	glutSwapBuffers();//ダブルバッファリング これがないとちらつく現象が...
}
