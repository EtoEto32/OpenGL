...
unsigned char texImage[imageHeight][imageWidth][3]; // テクスチャ画像データ
...
void readPPMImage(char* filename) {
    ...
    for (i = 0; i < 4; i++) { // ヘッダーの読み飛ばし
        while (1) {
            if ((ch = fgetc(fp)) != '#') break; // コメントをスキップ
            fgets((char*)texImage, 1024, fp); // ダミー読み込み
            while (isspace(ch)) ch = fgetc(fp); // 空白をスキップ
        }
        while (!isspace(ch)) ch = fgetc(fp); // ヘッダーを読み込み
        if (i < 3) {
            while (isspace(ch)) ch = fgetc(fp); // 区切り文字をスキップ
        }
    }
    fread(texImage, 1, imageWidth * imageHeight * 3, fp); // RGBデータを読み込み
    fclose(fp);
}

// テクスチャを設定する関数
void setUpTexture() {
    readPPMImage("sea.ppm");
...
// 球を発光させる関数
void makeSpheresGlow() {
    for (int i = 0; i < 3; i++) {
        sphere_color[i] = 1.0f; // 白色に設定
    }
    GLfloat glow_ambient[] = { 1.0f, 1.0f, 1.0f, 1.0f }; // 環境光成分を強化
...
void drawSphere(GLfloat x, GLfloat y, GLfloat z) {
    ...
    gluQuadricTexture(quadric, GL_TRUE); // テクスチャマッピングを有効にする
    gluSphere(quadric, 1.0, 50, 50); // 球を描画
...
void createMenu() {
    ...
    glutAddMenuEntry("luminescence", 2);
    glutAttachMenu(GLUT_RIGHT_BUTTON);
}
